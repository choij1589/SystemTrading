"""
5-Year US ETF Strategy Comparison (2020-2024)

Compare active strategies vs simple buy-and-hold benchmarks.
Focus on strategy effectiveness during recent market conditions.
"""
...
print(f"📦 Generating realistic data for {len(all_tickers)} ETFs...")
...
print(f"✓ Generated data for {len(data)} ETFs")

# Show key ETF statistics
print(f"\n✓ Key ETF Performance (5-year):")
...
ax.set_title('Portfolio Value: Active Strategies vs Benchmarks (2020-2024)',
...
plt.suptitle('5-Year Strategy Comparison (2020-2024): Active vs Benchmarks',
             fontsize=16, fontweight='bold', y=0.995)

# Save
output_path = 'ETFTrading/us_5year_comparison.png'
...
for r in results:
    ...
    r['equity'].to_csv(f"ETFTrading/5yr_{name}_equity.csv", index=False)

print("="*80)
print("KEY INSIGHTS (5-Year Period)")
print("="*80)
print()
...
print(f"   → Does rebalancing add value? {div_result['stats']['cagr'] > qqq_result['stats']['cagr']}")
print()
...
print(f"   → Diversification reduces drawdown by: {(spy_dd - aw_dd):.1%}pp")
print()
